...
class JSONParser(parsers.JSONParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        if isinstance(data, list):
            ...
            if in_relationship_view:
                ...
                parsed = self.parse_resource(data, result, resource_name, method, serializer_class)
    ...
    def verify_resource_identifier(self, data):
        if not data.get("id") or not data.get("type"):
            raise ParseError("Received data is not a valid JSON:API Resource Identifier Object(s).")
